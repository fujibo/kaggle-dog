...
class ResNetGenerator(chainer.Chain):
    def __init__(self, ch=64, dim_z=128, bottom_width=4, activation=F.relu, n_classes=0, distribution="normal", use_sn=False):
        ...
        with self.init_scope():
            ...
            self.block2 = Block(ch * 16, ch * 8, activation=activation, upsample=True, n_classes=n_classes, use_sn=use_sn)
            self.block3 = Block(ch * 8, ch * 4, activation=activation, upsample=True, n_classes=n_classes, use_sn=use_sn)
            self.block4 = Block(ch * 4, ch * 2, activation=activation, upsample=True, n_classes=n_classes, use_sn=use_sn)
            self.block5 = Block(ch * 2, ch, activation=activation, upsample=True, n_classes=n_classes, use_sn=use_sn)
